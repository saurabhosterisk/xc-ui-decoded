...
echo (($return["result"] ?? "") == "success" ? ($return["msg"] ?? "") : "");
...
function resellerapi_decrypt($q, $salt = NULL)
{
    ...
    $key = hash("sha256", (string) $salt, true);
    $iv = substr(hash("sha256", "iv:" . (string) $salt, true), 0, 16);
    $decoded = base64_decode(substr($q, 3), true);
    if ($decoded === false) {
        return "";
    }
...
function resellerapi_encrypt($q, $salt = NULL)
{
    $key = hash("sha256", (string) $salt, true);
    $iv = substr(hash("sha256", "iv:" . (string) $salt, true), 0, 16);
    $encrypted = openssl_encrypt((string) $q, "aes-256-cbc", $key, OPENSSL_RAW_DATA, $iv);
    return $encrypted === false ? "" : "v2:" . base64_encode($encrypted);
}
