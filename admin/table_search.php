...
$rType = $_GET["id"] ?? "";
$rStart = intval($_GET["start"] ?? 0);
$rLimit = intval($_GET["length"] ?? 1000);
$_GET["draw"] = $_GET["draw"] ?? 0;
$_GET["order"] = isset($_GET["order"][0]) && is_array($_GET["order"][0])
    ? $_GET["order"]
    : [["column" => 0, "dir" => "asc"]];
$_GET["order"][0]["column"] = $_GET["order"][0]["column"] ?? 0;
$_GET["order"][0]["dir"] = $_GET["order"][0]["dir"] ?? "asc";
$_GET["search"] = isset($_GET["search"]) && is_array($_GET["search"])
    ? $_GET["search"]
    : [];
$_GET["search"]["value"] = $_GET["search"]["value"] ?? "";
foreach (["filter", "category", "server", "reseller", "range"] as $rOptionalParameter) {
    $_GET[$rOptionalParameter] = $_GET[$rOptionalParameter] ?? "";
}
...
if ($rType == "users") {
    ...
    if (isset($_GET["order"][0]["column"]) && strlen((string) $_GET["order"][0]["column"]) > 0) {
        ...
        if ($rResult && 0 < $rResult->num_rows) {
            while ($rRow = $rResult->fetch_assoc()) {
                ...
                if ($rRow["type"] == 3) {
                    ...
                    $rSourceParts = parse_url((string) $rRow["current_source"]);
                    $rStreamSource = "<br/><span style='font-size:11px;'>" . ($rSourceParts["host"] ?? "") . "</span>";
                }
                if ($rPermissions["is_admin"]) {
                ...
                if ($rRow["type"] == 3 && count((array) json_decode($rRow["cchannel_rsources"], true)) != count((array) json_decode($rRow["stream_source"], true))) {
                    $rActualStatus = 6;
                }
                ...
                if (0 < strlen($rRow["stream_icon"])) {
                    $rIcon = "<img class=\"tv_logo\" src='".$rRow["stream_icon"] . "' />";
                } else {
                    $rIcon = "";
                }
                if ($rPermissions["is_admin"]) {
