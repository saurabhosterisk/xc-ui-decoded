...
class Release
{
    ...
    {
        $type = NULL;
        $title = preg_replace_callback("#S(?<season>\\d{1,2})E(?<episode>\\d{1,2})#i", function($matches)
{
    static $type = NULL;
    $type = self::TVSHOW;
...
, $title, 1, $count);
    ...
    {
        $year = NULL;
        $title = preg_replace_callback("#[\\.|\\-|\\(|\\ ](\\d{4})([\\.|\\-|\\|\\ )])?#", function($matches)
{
    static $year = NULL;
    if( isset($matches[1]) ) 
    {
...
, $title, 1);
    ...
    {
        $group = NULL;
        $title = preg_replace_callback("#\\-([a-zA-Z0-9_\\.]+)\$#", function($matches)
{
    static $group = NULL;
