...
if (!is_numeric($start)) {
    if (substr_count($start, '-') == 1) {
        ...
        list($hour, $minutes) = explode('-', $job);
    }
    $start_timestamp = mktime($hour, $minutes, 0, $month, $day, $year);
} else {
    $duration *= 24;
    $files = array_values(array_filter(explode('
', shell_exec('ls -tr ' . TV_ARCHIVE . $stream_id . ' | sed -e \'s/\\s\\+/\\n/g\''))));
    $archiveFile = $start * 24;
    if (count($files) >= $archiveFile) {
        $archiveFile = $files[count($files) - $archiveFile];
    } else {
        $archiveFile = $files[0];
    }
    if (preg_match('/(.*)-(.*)-(.*):(.*)\\./', $archiveFile, $matches)) {
    ...
    switch (ipTV_lib::$request['extension']) {
        case 'm3u8':
            if (isset($startOffset)) {
                if (!empty($queue[$startOffset]) && file_exists($queue[$startOffset]['filename']) && $queue[$startOffset]['filesize'] == $size) {
                    $fileOffset = 0;
                    $length = $queue[$startOffset]['filesize'];
                    if ($startOffset == 0) {
                        $fileOffset = $queue[$startOffset]['filesize'] * 0.3;
                        $length = $queue[$startOffset]['filesize'] - $fileOffset;
                    }
                    header('Content-Type: video/mp2t');
                    header('Content-Length: ' . $length);
                    $fp = fopen($queue[$startOffset]['filename'], 'r');
                    fseek($fp, $fileOffset);
                    while (!feof($fp)) {
...
';
        ...
        default:
            ...
            $startFileIndex = 0;
            if ($start > 0) {
                $startFileIndex = floor($start / ($size / count($queue)));
            }
            $offsetApplied = false;
            $fileOffset = 0;
            $totalBytes = 0;
            $bitrateBytesPerSecond = $bitrate * 125;
            $bitrateBytesPerSecond += $bitrateBytesPerSecond * ipTV_lib::$settings['vod_bitrate_plus'] * 0.01;
            $time_start = time();
            $bytesSent = 0;
            ...
            if (ipTV_lib::$settings['vod_limit_at'] > 0) {
                $maxBytesBeforeThrottle = intval($size * ipTV_lib::$settings['vod_limit_at'] / 100);
            } else {
                $maxBytesBeforeThrottle = $size;
            }
            $throttleReached = false;
            foreach ($queue as $k => $item) {
                $totalBytes += $item['filesize'];
                if (!$offsetApplied && $startFileIndex > 0) {
                    if ($startFileIndex > $k) {
                    } else {
                        $offsetApplied = true;
                        $fileOffset = $start - $totalBytes;
                    }
                }
                ...
                while (!feof($fp)) {
                    ...
                    ++$index;
                    if (!$throttleReached && $bufferIndex * $buffer >= $maxBytesBeforeThrottle) {
                        $throttleReached = true;
                    } else {
                        ++$bufferIndex;
                    }
                    if ($bitrateBytesPerSecond > 0 && $throttleReached && $index >= ceil($bitrateBytesPerSecond / $buffer)) {
                        sleep(1);
                        $index = 0;
                    }
                    if (time() - $time_start >= 30) {
                        file_put_contents($connection_speed_file, intval($bytesSent / 1024 / 30));
                        $time_start = time();
                        $bytesSent = 0;
                    }
                }
