...
try {
    ...
    // A missing/corrupt optional GeoIP database must not make streams fail.
}
$activity_id = 0;
...
if ($user_info = ipTV_streaming::GetUserInfo(null, $username, $password, true, false, true, array(), false, $user_ip, $user_agent, array(), $play_token, $stream_id)) {
    ...
    if ($geoip) {
        try {
            $geo = $geoip->getWithPrefixLen($user_ip);
            $geoip_country_code = $geo['registered_country']['iso_code'] ?? '';
        } catch (Throwable $e) {
            $geoip_country_code = '';
        }
        $geoip->close();
    ...
    if ($channel_info = ipTV_streaming::ChannelInfo($stream_id, $extension, $user_info, $user_ip, $geoip_country_code, $external_device, $user_info['con_isp_name'], 'live')) {
        ...
        switch ($extension) {
            case 'm3u8':
                ...
                break;
            default:
                // If the monitor PID is stale but a completed playlist/segment
                // set is present, serve the newest segment instead of waiting
                // forever for a segment number that will never be produced.
                if (!ipTV_streaming::ps_running($channel_info['pid'], FFMPEG_PATH) && file_exists($playlist)) {
                    $available = glob(STREAMS_PATH . $stream_id . '_*.ts') ?: array();
                    if ($available) {
                        usort($available, static function ($a, $b) { return filemtime($a) <=> filemtime($b); });
