...
if ($user_info = ipTV_streaming::GetUserInfo(null, $username, $password, true, false, true, array(), false, $user_ip, $user_agent, array(), $play_token, $stream_id)) {
    ...
    if ($channel_info = ipTV_streaming::ChannelInfo($stream_id, $extension, $user_info, $user_ip, $geoip_country_code, '', $user_info['con_isp_name'], 'movie')) {
        ...
        $bitrateBytesPerSecond = !empty($channel_info['bitrate']) ? $channel_info['bitrate'] * 125 : 0;
        $bitrateBytesPerSecond += $bitrateBytesPerSecond * ipTV_lib::$settings['vod_bitrate_plus'] * 0.01;
        $movieFile = MOVIES_PATH . $stream_id . '.' . $extension;
        if (file_exists($movieFile)) {
            $fp = @fopen($movieFile, 'rb');
            $size = filesize($movieFile);
            $length = $size;
            $start = 0;
            $end = $size - 1;
            ...
            if (ipTV_lib::$settings['vod_limit_at'] > 0) {
                $maxBytesBeforeThrottle = intval($length * ipTV_lib::$settings['vod_limit_at'] / 100);
            } else {
                $maxBytesBeforeThrottle = $length;
            }
