...
function getSeriesEpisodes($id, $fav = null, $orderby = null, $stream_display_name = null, $movie_properties = array())
{
    global $ipTV_db;
    ...
    $episodesBySeason = $ipTV_db->get_rows(true, 'season_num', false);
...
function GetSeriesOrderedList($movie_id = null, $category_id = null, $fav = null, $orderby = null, $stream_display_name = null, $movie_properties = array())
{
    ...
    $default_page = false;
    if (empty($movie_id)) {
        $movieResults = getSeriesForUser($dev['user_id'], $category_id, $fav, $orderby, $stream_display_name, $movie_properties);
    } else {
        $movieResults = getSeriesEpisodes($movie_id, $fav, $orderby, $stream_display_name, $movie_properties);
        $ipTV_db->query('SELECT * FROM `series` WHERE `id` = \'%d\'', $movie_id);
        $serie = $ipTV_db->get_row();
    }
    $counter = count($movieResults);
    $ch_idx = 0;
    if ($page == 0) {
        $default_page = true;
        $page = ceil($ch_idx / $page_items);
    ...
    $movieResults = array_slice($movieResults, ($page - 1) * $page_items, $page_items, true);
    $datas = array();
    foreach ($movieResults as $key => $movie) {
        if (!is_null($fav) && $fav == 1 && empty($movie_id)) {
            if (in_array($movie['id'], $dev['fav_channels']['series'])) {
                if (!empty($serie)) {
                    $movie_propeties = $serie;
                    $addedTimestamp = 0;
                    foreach ($movie as $episode) {
                        if ($episode['added'] > $addedTimestamp) {
                            $addedTimestamp = $episode['added'];
                        }
                    }
                } else {
                    $movie_propeties = $movie;
                    $addedTimestamp = $movie['last_modified'];
                }
                $cmd = array('type' => 'series', 'series_id' => $movie_id, 'season_num' => $key);
                $this_mm = date('m');
                $this_dd = date('d');
                $this_yy = date('Y');
                if (($addedTimestamp > mktime(0, 0, 0, $this_mm, $this_dd, $this_yy))) {
                    $added_key = 'today';
                    $added_val = $_LANG['today'];
                }
                else if (($addedTimestamp > mktime(0, 0, 0, $this_mm, $this_dd - 1, $this_yy))) {
                    $added_key = 'yesterday';
                    $added_val = $_LANG['yesterday'];
                }
                else if ($addedTimestamp > mktime(0, 0, 0, $this_mm, $this_dd - 7, $this_yy)) {
                    $added_key = 'week_and_more';
                    $added_val = $_LANG['last_week'];
                } else {
                    $added_key = 'week_and_more';
                    $added_val = date('F', $addedTimestamp) . ' ' . date('Y', $addedTimestamp);
                }
                if (!empty($serie)) {
                    if ($key == 0) {
...
function getSeriesForUser($user_id, $category_id = null, $fav = null, $orderby = null, $stream_display_name = null, $movie_properties = array())
{
    global $dev, $ipTV_db;
    ...
    switch ($orderby) {
        case 'name':
            uasort($seriesById, 'sortArrayStreamName');
            break;
        case 'top':
            uasort($seriesById, 'sortArrayStreamRating');
            break;
        case 'rating':
            uasort($seriesById, 'sortArrayStreamRating');
            break;
        case 'added':
            uasort($seriesById, 'sortArrayStreamAdded');
            break;
        case 'number':
            uasort($seriesById, 'sortArrayStreamNumber');
            break;
    }
    return $seriesById;
}
