...
class ipTV_streaming
{
                ...
                if (!(ipTV_lib::$settings['split_by'] == 'maxclients')) {
                    if (ipTV_lib::$settings['split_by'] == 'guar_band') {
                        ...
                        foreach ($StreamSysIds as $server_id) {
                            $hardware = json_decode(ipTV_lib::$StreamingServers[$server_id]['server_hardware'], true);
                            if (!empty($hardware['network_speed'])) {
                                $networkSpeeds[$server_id] = (double) $hardware['network_speed'];
                            } else {
                                $networkSpeeds[$server_id] = 1000;
                            }
                        }
                        foreach ($rows as $server_id => $row) {
                            $rows[$server_id]['capacity'] = (double) ($row['online_clients'] / $networkSpeeds[$server_id]);
                        }
                        foreach ($rows as $server_id => $row) {
    ...
    public static function GetUserInfo($user_id = null, $username = null, $password = null, $get_ChannelIDS = false, $getBouquetInfo = false, $get_cons = false, $type = array(), $includeAdult = false, $user_ip = '', $user_agent = '', $pagination = array(), $play_token = '', $stream_id = 0)
    {
        if (empty($user_id)) {
        ...
        if (self::$ipTV_db->num_rows() > 0) {
            ...
            if ($user_info['is_mag'] == 1 && ipTV_lib::$settings['mag_security'] == 1) {
                if (!empty($user_info['play_token']) && !empty($play_token)) {
                    list($token, $tokenExpiry, $tokenStreamId) = explode(':', $user_info['play_token']);
                    if (!($token == $play_token && $tokenExpiry >= time() && $tokenStreamId == $stream_id)) {
            ...
            if ($get_ChannelIDS) {
                ...
                foreach ($user_info['bouquet'] as $id) {
                    if (isset(ipTV_lib::$Bouquets[$id]['streams'])) {
                        $channelIds = array_merge($channelIds, ipTV_lib::$Bouquets[$id]['streams']);
                    }
                    if (isset(ipTV_lib::$Bouquets[$id]['series'])) {
                        $seriesIds = array_merge($seriesIds, ipTV_lib::$Bouquets[$id]['series']);
                    }
                }
                if (ipTV_lib::$settings['new_sorting_bouquet'] != 1) {
                    $user_info['channel_ids'] = array_unique($channelIds);
                    $user_info['series_ids'] = array_unique($seriesIds);
                } else {
                    $user_info['channel_ids'] = array_reverse(array_unique(array_reverse($channelIds)));
                    $user_info['series_ids'] = array_reverse(array_unique(array_reverse($seriesIds)));
                }
                if ($getBouquetInfo && !empty($user_info['channel_ids'])) {
                    ...
                    if (!empty($pagination['items_per_page'])) {
                        ...
                        $user_info['channels'] = array_slice($user_info['channels'], $pagination['offset'], $pagination['items_per_page']);
                    }
                }
            }
            return $user_info;
    ...
    public static function apiGetISPName($user_ip, $user_agent)
    {
        ...
        // ISP lookup is intentionally local-only; never transmit client IP or
        // user-agent data to a third-party endpoint.
        return false;
    }
    ...
    // Legacy monitor method aliases retained for running existing workers.
    public static function checkPidChannelM3u8Legacy($pid, $stream_id, $ffmpeg_path = FFMPEG_PATH)
    {
        return self::CheckPidChannelM3U8Exist($pid, $stream_id, $ffmpeg_path);
    }
    public static function checkPidStreamLegacy($pid, $stream_id)
    {
        return self::CheckPidStreamExist($pid, $stream_id);
    }
    public static function getStreamBitrateLegacy($path, $prebuffer = 0)
    {
        return self::GetStreamBitrate('live', $path, $prebuffer);
    }
}
?>
